...
setup() {
  ...
  echo "data" > "$PLUGIN_DATA_ROOT/fake.ldif"
}

teardown() {
  export ECHO_DOCKER_COMMAND="false"
  dokku --force "$PLUGIN_COMMAND_PREFIX:destroy" l >&2
  rm -f "$PLUGIN_DATA_ROOT/fake.ldif"
}
...
@test "($PLUGIN_COMMAND_PREFIX:import) success" {
  ...
  run dokku "$PLUGIN_COMMAND_PREFIX:import" l < "$PLUGIN_DATA_ROOT/fake.ldif"
  password="$(cat "$PLUGIN_DATA_ROOT/l/PASSWORD")"
  assert_output "docker exec -i dokku.ldap.l slapadd -c"
}
